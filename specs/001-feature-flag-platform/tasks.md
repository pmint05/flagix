...
- [X] T004 [P] Create `apps/backend/drizzle.config.ts` pointing at `src/db/schema/index.ts` with `dialect: 'postgresql'` and the `DATABASE_URL` from env
...
- [X] T009 Run Better Auth CLI `npx @better-auth/cli@latest generate --config apps/backend/src/modules/auth/auth.config.ts --output apps/backend/src/db/schema/auth-schema.ts` to auto-generate `user`, `session`, `account`, `verification` Drizzle table definitions; commit the generated file as-is
- [X] T010 Create empty platform schema directory `apps/backend/src/db/schema/` with `index.ts` barrel export (will be filled in Phase 2); verify `drizzle-kit generate` runs without errors against the empty file
...
- [X] T022 Create `apps/backend/src/common/decorators/current-user.decorator.ts` parameter decorator `@CurrentUser()` extending the `@thallesp/nestjs-better-auth` `@Session()` decorator to return the typed `User` object (or extract from `session.user`)
- [X] T023 Create `apps/backend/src/common/guards/org-roles.guard.ts` implementing `CanActivate`: reads `@OrgRoles(...)` metadata and `req.params.organizationId` (or header), queries `organizationMembers` for the caller's userId + organizationId, compares role; throws `ForbiddenException` on mismatch
- [X] T024 Create `apps/backend/src/common/guards/org-member.guard.ts` implementing `CanActivate`: verifies the authenticated user is a member of the org derived from the route (orgId from path/body); throws `ForbiddenException` if not
